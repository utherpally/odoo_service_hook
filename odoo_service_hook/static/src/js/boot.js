...
(function() {
    ...
    /* Start modified: add hook service utils */
    var post_hooks = {};

    var getServiceHookDeps = function(service) {
        if(service in post_hooks) {
            return post_hooks[service].deps;
        }
        return []
    }
    var getServiceHookCallbacks = function(service) {
        if(service in post_hooks) {
            return _.pluck(_.sortBy(post_hooks[service].callbacks, 'priority'), 'callback');
        }
        return []
    }

    /* End modified: add hook service utils */
    ...
    _.extend(odoo, {
        ...
        debug: debug,
        remaining_jobs: jobs,
        /* START MODIFIED: add main function using for service hook */
        post_define: function() {
            var args = Array.prototype.slice.call(arguments);
            var service = args.shift();
            var callback = args[args.length - 1];
            var priority = args[0] instanceof Number ? args[0] : 13;
            var deps = [];
            callback.toString()
            ...
            post_hooks[service].deps = post_hooks[service].deps.concat(deps)
            post_hooks[service].callbacks.push({
                priority: priority,
                callback: callback,
            });
        },
        /* END MODIFIED: add main function using for service hook */
        __DEBUG__: {
            didLogInfo: $.Deferred(),
        ...
        define: function () {
            ...
            var deps = getServiceHookDeps(name); // Get all service hook deps
            if (args[0] instanceof Array) {
                deps = deps.concat(args[0]);
            } else {
                factory.toString()
                    .replace(commentRegExp, '')
                    .replace(cjsRequireRegExp, function (match, dep) {
        ...
        process_jobs: function (jobs, services) {
            ...
            function process_job (job) {
                ...
                if (!job.error) {
                    $.when(job_exec).then(
                        function (data) {
                            // Run service hook before assign final data to `services`
                            _.each(getServiceHookCallbacks(job.name), function(callback) {
                                data = callback.call(null, data, require);
                            });
